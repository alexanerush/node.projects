...
app.get("/api/logic", authRequired, (req, res) => {
  res.json({
    ok: true,
    message: "Welcome to protected logic page",
    user: req.user,
  });
});
...
app.put("/api/articles/:id", authRequired, async (req, res, next) => {
  try {
...
app.post("/api/articles/:id/comments", authRequired, async (req, res, next) => {
  try {
    const { id } = req.params;
...
app.delete("/api/comments/:commentId", authRequired, async (req, res, next) => {
  try {
